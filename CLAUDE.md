...
- **天気予報表示**: `build`実行時にOpen-Meteo API（APIキー不要）から印西市の今日・明日の天気を取得し、ヒーロー記事の横に表示する。天気・気温は精度優先で気象庁(JMA)モデル(`models=jma_seamless`)を明示指定するが、このモデルは降水確率を返さないため、降水確率のみ既定モデル(best_match)へ別リクエストして補っている(計2回のAPI呼び出し)。取得失敗時は静かにスキップされ、ビルド自体は失敗しない。`news.json`には保存されない（build時の都度取得）。
