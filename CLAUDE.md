...
- **天気予報表示**: `build`実行時にウェザーニューズ(`weathernews.jp/onebox/tenki/chiba/12231/week.html?tab=4`)から印西市の今日・明日の天気をスクレイピングし、ヒーロー記事の横に表示する(クリックで同サイトの日次ページ`?tab=3`へ遷移)。このページはVue.js製だがサーバー側で実データがHTMLに埋め込まれているため、JS実行なしで取得可能（`#flick_list_today`内の`div.card`から天気文言・最高/最低気温・降水確率を抽出）。22時以降に実行された場合は「明日・明後日」を出そうとするが、`#flick_list_today`には通常「今日・明日」の2枚しかカードが無いため、明後日分が無ければ1日分のみ表示される(データ不足時は静かに縮退する)。取得失敗時は全体を静かにスキップし、ビルド自体は失敗しない。`news.json`には保存されない（build時の都度取得）。旧実装(Open-Meteo API)は精度への不満から2026-07-20に置き換えた。
