...
- **電車時刻表表示**: 2026-07-20にトップ記事のヒーロー表示を廃止し、代わりに北総鉄道(`hokuso.ekitan.com`)の印西牧の原駅・千葉ニュータウン中央駅の時刻表を表示するようにした(廃止に伴い、それまでヒーロー扱いだった記事は通常通り自分のカテゴリのグリッドに表示されるだけになった。記事が消えるわけではない)。表示するのは上り(京成高砂・日本橋方面)のみ、直近3本。番線は表記が不確実(印西牧の原は例外があり駅ごとに1つの番線に確定できない)なため表示していない。実際に「直近の電車」を判定するのは埋め込みデータではなくブラウザ側JS(閲覧者のPC/スマホがページを開いた時点のJST時刻、1秒ごとに再計算して残り時間をカウントダウン表示)で、平日/土曜・休日の判定は曜日に加えて埋め込んだ祝日リストも見る(祝日は土曜・休日ダイヤ扱い)。行き先の略字(例:「羽」→羽田空港)や「当駅始発」(●マーク、ページ内で下線表記になっている電車)はページ内の凡例・HTML構造から都度読み取って変換するため、ダイヤ改正で内容が変わっても追従できる。特急・アクセス特急はオレンジ色で表示。
  - **キャッシュ**: ダイヤ改正は年に数回程度しかないため、`build`のたびに北総鉄道サイトへ問い合わせるのは過剰と判断し、`load_or_fetch_train_data()`が`train_timetable_cache.json`(gitignore対象、ローカルのみ)に取得結果を保存する。キャッシュが`TRAIN_CACHE_MAX_AGE_DAYS`(30日)以内ならネットワークアクセスなしでそれを使い、古い/存在しない場合のみ再取得する。再取得が全滅した場合(サイト障害等)は多少古くてもキャッシュがあればそれを使い続ける(空表示になるより古い時刻表の方がまし)。祝日データも同じキャッシュに同梱される。取得失敗時は駅ごとに空リストのまま(その駅だけ表示されない)。
  - **ダイヤ改正検知**: キャッシュが古くなって再取得した際、内容が前回キャッシュと異なっていたら`TRAIN_TIMETABLE_CHANGED`を立て、`build`実行時に「北総鉄道の電車時刻表が前回取得時から変更されています(ダイヤ改正の可能性)」と標準出力に表示する。「ニュース更新して」の手順内でこの出力を見かけたら、通常の件数サマリとは別にユーザーへ一言報告すること。
