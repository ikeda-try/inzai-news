...
- **電車時刻表表示**: 2026-07-20にトップ記事のヒーロー表示を廃止し、代わりに電車時刻表ウィジェットを表示するようにした(廃止に伴い、それまでヒーロー扱いだった記事は通常通り自分のカテゴリのグリッドに表示されるだけになった。記事が消えるわけではない)。直近3本を表示。番線は表記が不確実(印西牧の原は例外があり駅ごとに1つの番線に確定できない)なため表示していない。実際に「直近の電車」を判定するのは埋め込みデータではなくブラウザ側JS(閲覧者のPC/スマホがページを開いた時点のJST時刻、1秒ごとに再計算して残り時間をカウントダウン表示)で、平日/土曜・休日の判定は曜日に加えて埋め込んだ祝日リストも見る(祝日は土曜・休日ダイヤ扱い)。当駅始発の電車には●マークを付ける。特急・アクセス特急はオレンジ色で表示。
  - **上り/下り切り替え**: 2026-07-21に、ヘッダー(「印西ニュース」ロゴと最終更新の間)にトグルボタンを追加し、2方向を切り替えられるようにした。
    - 上り(初期表示): 印西牧の原駅・千葉ニュータウン中央駅→京成高砂・日本橋方面。`hokuso.ekitan.com`(北総鉄道公式の時刻表サイト)から取得(`TRAIN_STATIONS_UP`, d=1)。行き先の略字(例:「羽」→羽田空港)や「当駅始発」(ページ内で下線表記になっている電車)はページ内の凡例・HTML構造から都度読み取って変換するため、ダイヤ改正で内容が変わっても追従できる。
    - 下り: 浅草橋駅・新鎌ヶ谷駅→印西牧の原・成田空港方面(通勤で印西に帰ってくる方向)。新鎌ヶ谷は北総線内なので同じく`hokuso.ekitan.com`(`TRAIN_STATIONS_DOWN`, d=2)。浅草橋は北総線の駅ではなく`hokuso.ekitan.com`の検索対象外のため、一般の`ekitan.com`(駅探、路線コード`222-15`)から`fetch_train_timetable_ekitan()`で別途取得している。このサイトは方面ごとに時刻表テーブルが2つ並んで出力され、タブの並び順(1つ目=西馬込方面、2つ目=押上方面)通りに出現するため2つ目のテーブルを使う。行き先・種別・当駅始発フラグはいずれも`data-dest`/`data-tr-type`/`data-start`属性から直接取れる(凡例展開が不要)ため、hokuso側より単純。曜日区分はhokuso側がdw=0(平日)/dw=1(土曜・休日)の2区分なのに対し、ekitan.com側はdw=0(平日)/dw=1(土曜)/dw=2(休日)の3区分。既存の平日/週末2区分ロジックに合わせるため週末側はdw=2(休日)を採用している(土曜と休日で若干本数が異なるが実用上の近似として許容)。
    ...
    - サーバー側は上り・下り両方の駅カラムを常にHTMLに埋め込み(`train-columns-up`/`train-columns-down`)、非表示側は`train-columns-hidden`クラスで隠すのみ。トグルボタンのJSがクラス切り替え・タイトル文言(「北総鉄道」⇔「浅草橋・新鎌ヶ谷」等)・カウントダウン再描画をまとめて行う。
